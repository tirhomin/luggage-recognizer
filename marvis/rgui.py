...
from marvis import torchlib as nnlib #nnlib can be either our torchlib or our tflib
...
class GUI(object):
    ...
    def run(self,videosupport=False,debug=False,cpulimit=False):
        ...
        if self.videosupport:
            ...
            self.cvthread.start()  
        self.tthread = threading.Thread(target=nnlib.tfworker,args=(self.tque,self.framequeue,cpulimit))
        ...
        self.debug=debug
        if self.debug:
            self.root.after(500,self.set_image,'demo6.jpg')
        self.root.mainloop()
#----------------------
